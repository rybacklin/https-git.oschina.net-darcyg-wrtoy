...
git_prefix=""
if [ -f "./usegit" ]; then
  git_prefix=$(cat ./usegit)
  git_targz=$(cat ./usegit | sed -e 's/works\//openwrt_/').tar.gz
  show -i "当前版本工作目录：$git_prefix 备份文件:$git_targz"
fi

if [ -f "backup/$git_targz" ]; then
  cp backup/$git_targz ./$git_targz
fi

if [ -f ./$git_targz ]; then

  if [ -d $git_prefix ]; then
    show -i "删除当前项目openwrt"
    rm -Rf $git_prefix
  fi
